            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
                <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-xl">
                    ...
                    <table className="min-w-full mb-6">
                        ...
                        <tbody>
                            {Object.keys(fieldMappings).map(dbField => (
                                <tr key={dbField}>
                                    <td className="px-2 py-1 text-xs font-medium text-gray-600">{dbField}{fieldMappings[dbField].required && <span className="text-red-500 ml-1">*</span>}</td>
                                    <td className="px-2 py-1">
                                        <select value={mapping[dbField]} onChange={(e) => handleMappingChange(dbField, e.target.value)} className="w-full p-1 border rounded-md bg-white text-xs">
                    ...
                    <div className="flex justify-end space-x-4">
                        ...
                        <button onClick={handlePreview} className="px-4 py-2 bg-blue-600 text-white rounded-md">Preview Import</button>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
                <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-xl">
                    ...
                    <div className="max-h-32 overflow-y-auto bg-gray-100 rounded-md p-2 mb-4 text-xs font-mono">
                        ...
                        {preview.toSkip.slice(0, 3).map((item, idx) => <div key={idx}>Skip: {JSON.stringify(item)}</div>)}
                    </div>
                    <div className="flex justify-end space-x-4">
                        <button onClick={() => setStep('mapping')} className="px-4 py-2 bg-gray-200 rounded-md">Back</button>
                        <button onClick={handleImport} disabled={isProcessing} className="px-4 py-2 bg-green-600 text-white rounded-md">{isProcessing ? 'Importing...' : 'Import Data'}</button>
                    </div>
                </div>
            </div>
        );
